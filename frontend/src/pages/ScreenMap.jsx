...
export default function ScreenMap() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await API.get("/screens/public");
        if (!cancelled) {
          setScreens(Array.isArray(res.data) ? res.data : []);
        }
      } catch {
        if (!cancelled) {
          // Fall back to curated static pins if API is unavailable
          setScreens(images.screens);
          setError("Showing demo screens — live network temporarily unavailable.");
        }
      } finally {
  ...
  return (
    <PublicLayout>
      <section className="bg-dark py-16 text-white">
        <div className="container-page">
          ...
          <p className="mt-3 max-w-xl text-gray-400">
            Explore live AdMax screens across India. Click a pin for details.
          </p>
        </div>
      </section>
      ...
      <div className="container-page py-10">
        ...
        {loading ? (
          <PageLoader label="Loading screen network..." />
        ) : (
              ...
              {filtered.map((screen) => (
                ...
                >
                  <div className="flex gap-3">
                    <img
                      src={screen.image || images.home.featureScreens}
                      alt={screen.name}
